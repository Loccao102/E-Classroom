...
type TabKey = 'dashboard' | 'admin' | 'teacher' | 'parent' | 'student' | 'communication' | 'notifications'
...
export default function App() {
  ...
  const signOut = () => { tokens.clear(); setMe(null); setSchoolId(''); setTab('dashboard') }
  const openTab = (next: TabKey) => { setTab(next); setMobileNavOpen(false) }
  ...
  return <div className="product-shell">
    ...
    <aside className={`product-sidebar ${mobileNavOpen ? 'mobile-open' : ''}`}>
      ...
      <div className="school-context"><span>Không gian hiện tại</span><select aria-label="Chọn trường" value={schoolId} onChange={event => { setSchoolId(event.target.value); setTab('dashboard') }}>{uniqueSchools.map(item => <option key={item.school_id} value={item.school_id}>{item.school_name}</option>)}</select></div>
      ...
      <div className="sidebar-footer"><div className="user-avatar">{initials(me.user.full_name)}</div><div><strong>{me.user.full_name}</strong><span>{roleSummary(roles, me.user.platform_role)}</span></div><button className="text-button" onClick={signOut}>Đăng xuất</button></div>
    </aside>

    <main className="product-main">
      <header className="mobile-header"><button className="menu-button" aria-label="Mở điều hướng" onClick={() => setMobileNavOpen(value => !value)}>☰</button><div className="brand compact"><div className="brand-symbol">EC</div><strong>E-Classroom</strong></div><button className="notification-shortcut" aria-label={`${unreadCount} thông báo chưa đọc`} onClick={() => openTab('notifications')}>{unreadCount || '•'}</button></header>
      <div className="context-header"><div><span>{currentSchool?.school_name || 'E-Classroom'}</span><strong>{nav.find(item => item.key === tab)?.label}</strong></div><div className="context-actions"><span className="role-chip">{roleSummary(roles, me.user.platform_role)}</span></div></div>
      <div className="page-stage">
        {!schoolId ? <div className="empty">Tài khoản chưa có thành viên trường học.</div> : <>
