...
public class BulkExportService {
    ...
    public ExportFile export(UUID schoolId,UUID actor,String requestedType,String requestedFormat,UUID classroomId,LocalDate from,LocalDate to){
        String type=normalizeType(requestedType),format=normalizeFormat(requestedFormat);
        if(Set.of("STUDENTS","TEACHERS","GUARDIANS").contains(type))access.requireAnyRole(schoolId,actor,"SCHOOL_ADMIN");
        ...
        audit(schoolId,actor,type,classroomId,from,to,format,data.rows().size());
    ...
    private void audit(UUID school,UUID actor,String type,UUID scope,LocalDate from,LocalDate to,String format,int count){try{String params=json.writeValueAsString(Map.of("from",from==null?"":from.toString(),"to",to==null?"":to.toString()));jdbc.update("INSERT INTO integration.export_audit(id,school_id,export_type,scope_type,scope_id,parameters,format,row_count,exported_by) VALUES (?,? ,?,'CLASSROOM',?,CAST(? AS jsonb),?,?,?)",UUID.randomUUID(),school,type,scope,params,format,count,actor);}catch(Exception ex){throw new IllegalStateException("Cannot audit export",ex);}}
