...
@RestController @RequestMapping("/api/v1/schools/{schoolId}")
public class AcademicController {
    private final AcademicService service; private final AccessService access;
    ...
    @GetMapping("/{resource:academic-years|semesters|subjects|teachers|students|guardians|classrooms|teaching-assignments|timetable}") public List<Map<String,Object>> list(@PathVariable UUID schoolId,@PathVariable String resource,Authentication a){access.requireMembership(schoolId,CurrentUser.id(a));return service.list(schoolId,resource);}
    ...
    @GetMapping("/classrooms/{classroomId}/roster") public List<Map<String,Object>> roster(@PathVariable UUID schoolId,@PathVariable UUID classroomId,Authentication a){access.requireMembership(schoolId,CurrentUser.id(a));return service.roster(schoolId,classroomId);}

    public record Year(String name,LocalDate startDate,LocalDate endDate){} public record Semester(UUID academicYearId,String name,LocalDate startDate,LocalDate endDate){} public record Grade(String name,int sortOrder){} public record Subject(String code,String name){}
    public record Teacher(@NotBlank String code,@NotBlank String fullName,String email,String phone,String password){} public record Student(@NotBlank String code,@NotBlank String fullName,LocalDate dateOfBirth,String gender,String email,String password){} public record Guardian(@NotBlank String fullName,String email,String phone,String password){}
    public record LinkGuardian(@NotNull UUID studentId,@NotNull UUID guardianId,@NotBlank String relationship,boolean primaryContact){} public record Classroom(UUID academicYearId,UUID gradeLevelId,String code,String name,UUID homeroomTeacherId){} public record Enrollment(UUID classroomId,UUID studentId,LocalDate startDate){} public record Assignment(UUID teacherId,UUID classroomId,UUID subjectId,UUID semesterId){} public record Timetable(UUID teachingAssignmentId,int weekday,int period,String room,LocalDate validFrom,LocalDate validTo){}
}
